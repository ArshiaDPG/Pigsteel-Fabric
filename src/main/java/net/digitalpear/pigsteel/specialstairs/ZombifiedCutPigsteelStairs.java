...
public class ZombifiedCutPigsteelStairs extends StairsBlock{
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit){
        if (player.getStackInHand(hand).getItem() == Items.HONEYCOMB) {
            ...
            world.setBlockState(pos, PigsteelBlocks.WAXED_ZOMBIFIED_CUT_PIGSTEEL_STAIRS.getDefaultState().with(Properties.HORIZONTAL_FACING, direction).with(Properties.WATERLOGGED, waterred).with(Properties.STAIR_SHAPE, shape).with(Properties.BLOCK_HALF, half));
